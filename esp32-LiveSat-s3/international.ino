...
static void handleTestStb() {
    ...
    s_wifiPortalServer.send(200, "text/plain", msg);
    appendDiagLog("teststb: len=%u prog=%d ok=%d ms=%lu psram=%u err=%s hdr=%02X%02X%02X%02X\n",
...
static void handleTestDl() {
    ...
    appendDiagLog("testdl: begin http\n");
    http.begin(client, url);
    http.setTimeout(15000);
    appendDiagLog("testdl: GET start\n");
    int code = http.GET();
    appendDiagLog("testdl: GET done code=%d\n", code);
    if (code != HTTP_CODE_OK) {
        char msg[64];
